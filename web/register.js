...
async function recordWav(seconds = 8) {
...
async function registerUser() {
  ...
  let json = null;
  try { json = JSON.parse(text); } catch (_) { json = { ok: false, error: 'Non-JSON response', raw: text.slice(0, 200) }; }
  return { res, json };
}
...
$('btnRegister').addEventListener('click', async () => {
  try {
    $('result').textContent = 'Result: registering…';
    const { res, json } = await registerUser();

    if (!res.ok || json.ok === false) {
      $('result').textContent = `Result: failed (${res.status}) ${json.error || ''}`;
      return;
    }

    $('result').textContent = `Result: REGISTERED — ${json.user?.name || ''} (${json.user?.email || ''})`;
  } catch (e) {
